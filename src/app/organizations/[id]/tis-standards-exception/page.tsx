...
export default function TISStandardsExceptionPage() {
  const params = useParams()
  const router = useRouter()
  const { selectedClient } = useClient()
  const [organization, setOrganization] = useState<Organization | null>(null)
  const [exceptions, setExceptions] = useState<TISStandardsException[]>([])
  const [loading, setLoading] = useState(true)
  const [searchTerm, setSearchTerm] = useState('')
  const [filteredExceptions, setFilteredExceptions] = useState<TISStandardsException[]>([])
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('does not exist') || error.message.includes('relation') || error.code === 'PGRST116') {
          console.warn('TIS standards exceptions table does not exist yet. Migration may be required.')
  ...
  const filterExceptions = () => {
    if (!searchTerm) {
      ...
      const filtered = exceptions.filter(exception =>
        ...
        exception.standard_reference.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="flex">
          ...
          <div className="flex-1 p-6">
            <div className="flex items-center justify-center h-64">
              <div className="text-gray-400">Loading TIS standards exceptions...</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-semibold">TIS Standards Exception</h1>
            ...
            <div className="flex items-center space-x-2">
              <button 
                onClick={() => router.push(`/organizations/${params.id}/tis-standards-exception/new`)}
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-yellow-100 text-sm mb-3">
                    The TIS Standards Exceptions table doesn't exist yet. This is expected after renaming from F12 to TIS.
                  </p>
              ...
              <table className="w-full">
                <thead className="bg-gray-700">
                  <tr>
                    ...
                    <th className="text-left p-4 text-sm font-medium text-gray-300">Standard</th>
                  ...
                  {filteredExceptions.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="text-center p-8 text-gray-400">
                        No TIS standards exceptions found
                      </td>
                    </tr>
                  ) : (
                    filteredExceptions.map((exception) => (
                      <tr key={exception.id} className="hover:bg-gray-700/50">
                        ...
                        <td className="p-4 text-gray-300">{exception.standard_reference}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded-full text-xs ring-1 capitalize ${getSeverityColor(exception.severity)}`}>
                            {exception.severity}
                          </span>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-gray-300">{formatDate(exception.created_at)}</td>
                        <td className="p-4">
                          <div className="flex items-center space-x-2">
